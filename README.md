...
### 核心库存管理
* **全面的CRUD操作**: 对物品信息进行流畅的增、删、改、查。
* **强大的数据筛选**: 支持按关键词（名称、供应商等）和类别进行组合搜索。
* **动态排序**: 支持点击任意列标题对数据进行排序。

### 数据洞察与处理
* **实时数据统计**: 一键查看库存总览，包括物品总类、总估值，以及按类别/状态的详细分布。
* **导出至CSV**: 可将当前数据视图一键导出为CSV文件，方便在Excel等工具中进行分析和共享。

### 高度可扩展性 (插件系统)
* **动态动作插件**: 无需重新编译主程序，即可通过添加`.dll`/`.so`文件来扩展新功能（例如已实现的“**从JSON批量导入**”功能）。
* **动态UI插件**: 支持通过插件加载全新的用户界面模块（例如已实现的“**数据仪表盘**”视图），可将程序的核心工作区完全替换。

### 现代化用户体验
* **联网自动更新**: 程序可自动连接到指定的GitHub仓库，检查最新发布的版本，并提供下载和更新提示。
* **个性化主题**: 支持用户从本地文件中**自由选择图片**作为程序背景，并自动保存用户的选择。
* **图形化设置**: 提供直观的设置对话框，用于修改和测试MySQL数据库的连接参数。
* **专业外观**: 拥有统一的应用程序图标，提升品牌感。

### 健壮的软件架构
* **分层设计**: 采用类似MVC的设计模式，将UI、业务逻辑和数据访问（`DatabaseManager`）清晰分离。
* **单例模式**: 通过单例模式管理数据库连接（`DatabaseManager`）和应用配置（`SettingsManager`），确保全局唯一性。
* **安全的外部化配置**: 数据库密码等敏感信息存储在外部`config.ini`文件中，而非硬编码在代码里。
* **跨平台构建**: 使用 **CMake** 作为构建系统，理论上可轻松迁移至Linux、macOS等平台。
* **资源嵌入式部署**: 图标、图片等资源通过Qt资源系统（`.qrc`）编译进程序，简化了最终部署。
...
* **编程语言**: C++ 17
* **框架**: Qt 6 (核心模块: Widgets, Sql, Network)
* **构建系统**: CMake
* **数据库**: MySQL
